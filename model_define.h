...
const REAL IF_GRID_SPACING = 3750; // micro meters
...
typedef enum _cell_state_real_e {
        ...
        CELL_MODEL_REAL_ATTACHMENT_X,
        CELL_MODEL_REAL_ATTACHMENT_Y,
        CELL_MODEL_REAL_ATTACHMENT_Z,
        NUM_CELL_STATE_REALS
} cell_state_real_e;

typedef enum _cell_state_int_e { 
        CELL_MODEL_INT_STATE,
        CELL_MODEL_INT_BOND_B,
        NUM_CELL_STATE_INTS
} cell_state_int_e ;
...
typedef enum _grid_summary_real_e {
        ...
        GRID_SUMMARY_REAL_LIVE_ATTACHED,
        GRID_SUMMARY_REAL_DEATH,
        GRID_SUMMARY_REAL_REMOVED,
...
typedef enum _grid_model_real_e {
        GRID_MODEL_REAL_DEATHCELL_COUNT,
...
// parameters of bonding with supporting surface.
const REAL A_AGENT_BOND_BOUNDARY_CREATE[NUM_MECH_TYPES] ={0.0, 1.1, 1.0};
const REAL A_AGENT_BOND_BOUNDARY_DESTROY[NUM_MECH_TYPES] ={0.0, 1.1, 1.0};
const REAL A_AGENT_BOND_BOUNDARY_S[NUM_MECH_TYPES] ={0.2, 0.2, 0.2};
const REAL A_AGENT_BOUNDARY_STIFFNESS[NUM_MECH_TYPES] ={1e-3 , 1e-3 , 1e-3 };
...
const REAL A_MCARRIER_DENSITY_PER_UB =  1 / (4.0 *4.0 * 4.0 ) ;  // 2000 //0.02 ; // 0.1
...
const REAL BASELINE_TIME_STEP_DURATION = 0.1  ; //0.0001; //  seconds
const REAL STEP_TIME = 1.0 ;
const REAL NUM_STATE_AND_GRID_TIME_STEPS_PER_BASELINE = 1 ;

// cell growth
const REAL DOUBLING_TIME = 0.1 ;   //BASELINE_TIME_STEP_DURATION * 100000 ; // I also used 100000
...
const REAL ODE_CELL_GROWTH_CONSTANT = 0.0 ; 
const REAL STRESS_TRESHOLD = 1e-6 ;  // 1e-7 ; //1e-7 ; // 
const REAL STRESS_HILL_EXPONENT = 5.0 ;
const REAL STRESS_TRESHOLD_OVER_N = POW( STRESS_TRESHOLD , STRESS_HILL_EXPONENT ) ;
 
// Bioreactor Geometry
const REAL PLATE_HEIGTH = 4100.0; // mmicromenters 
const REAL DIST_INTER_PLATE = 3250.0; //
const S32 NUM_PLATES = 8 ;
...
// Verlet integration
const S32 AGENT_TRANSLATION_ROTATION_INTEGRATION_STEPS_PER_BASELINE_TIME_STEP = 10000 ;
...
const REAL VELOCITY_DAMPING_TEST  = 1.0; // 1.0; //1.0e-4;

// gravity acceleration
const REAL STANDARD_GRAVITY = 9.80665e-6 ; // micro meter / ( s^2 )

// cell death parameters 
const REAL MECH_STRESS_TRESHOLD_DEATH = 1e-6 ;// 2.5e-7 ;  //  2.5e-7 ;//5e-7 ;
const REAL TIME_TO_DEATH = 0.1 *  DOUBLING_TIME   ;   
