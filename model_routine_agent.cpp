...
if( init == true ) {
   ...
   for( S64 j = 0 ; j < NUM_PLATES  ; j++ ) {
   //for( S64 j = 0 ; j < 1  ; j++ ) {
  
      REAL x = PLATE_HEIGTH + ( (REAL)(j)  * DIST_INTER_PLATE ) ; 

      for (  S64 i = 0 ; i < numCells  ; i++ ) {
         ...
         if ( ( vPos[1] < 3050.0 )  ||  ( vPos[1] > 26950.0  )  ) {
         ...
         state.setType( AGENT_CELL_A );
         state.setModelReal( CELL_MODEL_REAL_RADIUS, cellrad );
         ...
         state.setModelReal( CELL_MODEL_REAL_ATTACHMENT_X , x ) ;
         state.setModelReal( CELL_MODEL_REAL_ATTACHMENT_Y , vPos[1] ) ;
         state.setModelReal( CELL_MODEL_REAL_ATTACHMENT_Z , vPos[2] ) ;


         state.setODEVal(0, ODE_NET_VAR_GROWING_CELL_BIOMASS, biomass );
         state.setODEVal(0, ODE_NET_VAR_STRESS_TIME, 0.0 );

         state.setModelInt( CELL_MODEL_INT_STATE, CELL_A_LIVE ) ; 
         state.setModelInt( CELL_MODEL_INT_BOND_B, 0  ) ;
 
         state.setMechIntrctBdrySphere( A_CELL_D_MAX[ AGENT_CELL_A ] );
...
void ModelRoutine::spAgentCRNODERHS( const S32 odeNetIdx, const VIdx& vIdx, const SpAgent& spAgent, const NbrUBEnv& nbrUBEnv, const Vector<double>& v_y, Vector<double>& v_f ) {
	/* MODEL START */
        ...
        if ( mtype == CELL_A_LIVE )  { 
           ...
           if ( mech_stress < 0.0 )   { 
              mech_stress = -1.0 * mech_stress ;
           }

           //REAL factor = POW( mech_stress , STRESS_HILL_EXPONENT  ); 
           REAL factor = STRESS_TRESHOLD_OVER_N/(STRESS_TRESHOLD_OVER_N + POW( mech_stress , STRESS_HILL_EXPONENT)  );
...
void ModelRoutine::updateSpAgentState( const VIdx& vIdx, const JunctionData& junctionData, const VReal& vOffset, const NbrUBEnv& nbrUBEnv, SpAgentState& state/* INOUT */ ) {
    ...
    if ( type == AGENT_CELL_A )  {
        ...
        S32 mtype = state.getModelInt( CELL_MODEL_INT_STATE ) ;
        if ( A_AGENT_BOND_BOUNDARY_S[mtype] > 0.0 ) {

           REAL x = IF_GRID_SPACING*( (REAL)vIdx[0] + 0.5) + vOffset[0];
           REAL y = IF_GRID_SPACING*( (REAL)vIdx[1] + 0.5) + vOffset[1];
           REAL z = IF_GRID_SPACING*( (REAL)vIdx[2] + 0.5) + vOffset[2];

           REAL xo = state.getModelReal( CELL_MODEL_REAL_ATTACHMENT_X );
           REAL yo = state.getModelReal( CELL_MODEL_REAL_ATTACHMENT_Y );
           REAL zo = state.getModelReal( CELL_MODEL_REAL_ATTACHMENT_Z );

           REAL dist = SQRT( (x-xo)*(x-xo) + (y-yo)*(y-yo) + (z-zo)*(z-zo) ) ;

           REAL R0 = A_AGENT_SHOVING_SCALE[mtype]*state.getModelReal( CELL_MODEL_REAL_RADIUS ) ;

           if ( dist > R0*A_AGENT_BOND_BOUNDARY_DESTROY[mtype] ) {
               state.setModelInt(CELL_MODEL_INT_BOND_B,0);
           }
        }        


        if ( (newRadius <= A_MIN_CELL_RADIUS[type] ) && (state.getModelReal( CELL_MODEL_REAL_UPTAKE_PCT) == 1.0) ) 
            state.setModelReal( CELL_MODEL_REAL_UPTAKE_PCT, 0.0 ) ;
        else if (( newRadius <= A_MIN_CELL_RADIUS[type] ) && (state.getModelReal( CELL_MODEL_REAL_UPTAKE_PCT) == 0.0))
            state.setModelReal( CELL_MODEL_REAL_UPTAKE_PCT, -1.0 ) ;
    }
    ...
    if ( type == AGENT_CELL_A )  {
        ...
        if ( cell_rad >= testrad ) {
            if ( spAgent.state.getModelInt(CELL_MODEL_INT_STATE) == CELL_A_LIVE ) {     
                if ( spAgent.state.getModelInt( CELL_MODEL_INT_BOND_B ) == 1 ) {
                        divide = true;
                }
            }
        }
      
        else if ( spAgent.state.getModelReal( CELL_MODEL_REAL_UPTAKE_PCT) == -1.0 ) {
            //cout << "seeee " << cell_rad << "  " << type <<  endl;
            disappear = true;
        }

        REAL z =  IF_GRID_SPACING *( (REAL)(vIdx[2])  +  0.5)  + spAgent.vOffset[2];
        REAL Lz =  ( REAL ) ( Info::getDomainSize(2) ) * IF_GRID_SPACING ;
        if ( z < cell_rad ) {
           disappear = true ;
        }
        else if ( z > (Lz - cell_rad) ) {
           disappear = true ;
        }
        

        // remove ells if hit simulation domain
    }
...
void ModelRoutine::adjustSpAgent( const VIdx& vIdx, const JunctionData& junctionData, const VReal& vOffset, const MechIntrctData& mechIntrctData, const NbrUBEnv& nbrUBEnv, SpAgentState& state/* INOUT */, VReal& vDisp ) {/* if not dividing or disappearing */
 ...
 // }
  ...
  if ( state.getModelInt(CELL_MODEL_INT_BOND_B) == 1) {
        ...
        VReal vDir ;
        REAL att_x = state.getModelReal( CELL_MODEL_REAL_ATTACHMENT_X );
        REAL att_y = state.getModelReal( CELL_MODEL_REAL_ATTACHMENT_Y );
        REAL att_z = state.getModelReal( CELL_MODEL_REAL_ATTACHMENT_Y );  
        ...
        REAL dist = vDir.length() ;
        vDir = vDir / dist ;

        //REAL dist_b = FABS(  );
        REAL R0 = A_AGENT_SHOVING_SCALE[mtype]*state.getModelReal( CELL_MODEL_REAL_RADIUS ) ;
        REAL xij = R0 - dist;
        REAL mag = xij * tanh( FABS(xij) * A_AGENT_BOND_BOUNDARY_S[mtype] );
        stress = stress + A_AGENT_BOUNDARY_STIFFNESS[mtype]  * dist * mag ;

        vForce[0] += A_AGENT_BOUNDARY_STIFFNESS[mtype] * vDir[0]*mag ;
        vForce[1] += A_AGENT_BOUNDARY_STIFFNESS[mtype] * vDir[1]*mag ;
        vForce[2] += A_AGENT_BOUNDARY_STIFFNESS[mtype] * vDir[2]*mag ;
  }
